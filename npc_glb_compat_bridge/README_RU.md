# Zonecraft NPC Stalker Compatibility Bridge 1.2.1
...
- Исправлен аварийный `java.lang.NoSuchFieldError: entity` при загрузке мира: мост теперь собран с точными типами полей `EntityJoinWorldEvent.entity` и `EntityJoinWorldEvent.world` для Forge 1.6.4.
...
- После опустошения магазина NPC ждёт штатное время перезарядки оружия, затем продолжает стрелять.
...
1. Удалить старые версии:
   ...
   - `Zonecraft-NPC-Stalker-Compatibility-Bridge-1.6.4-1.2.0.jar`.
2. Положить `Zonecraft-NPC-Stalker-Compatibility-Bridge-1.6.4-1.2.1.jar` в папку `mods`.
...
```text
[Zonecraft NPC GLB Compat] 1.2.1 initialized: Forge event descriptor fix + real ItemWeapon cadence/reload + no ammo-box projectiles + all-faction vortex physics + held weapon render.
```
