...
/**
 * Starts an already installed Minecraft/Forge 1.6.4 client without contacting
 * the legacy LaunchServer. This class does not download or repair game files.
 */
public final class LocalOfflineLauncher {
    ...
    public static Process launch(Path updatesDir, String profileName, String username,
        ...
        if (classPath.isEmpty()) {
            throw new IOException("No JAR files were found in local client directory: " + clientDir);
        }
        ...
        } else if (containsClass(classPath, MODERN_MAIN_CLASS)) {
            mainClass = "net.minecraft.client.main.Main";
        } else if (containsClass(classPath, LEGACY_MAIN_CLASS)) {
            mainClass = "net.minecraft.client.Minecraft";
        } else {
            throw new IOException("Minecraft main class was not found in: " + clientDir);
        }

        Path javaBin = resolveJavaBinary();
        Path assetsDir = firstExistingDirectory(
            clientDir.resolve("assets"),
            normalizedUpdatesDir.resolve("assets"),
            clientDir.resolve("resources"),
            normalizedUpdatesDir.resolve("resources"),
            clientDir
        );
        Path nativesDir = firstExistingDirectory(
            clientDir.resolve("natives"),
            clientDir.resolve("bin").resolve("natives"),
            normalizedUpdatesDir.resolve("natives"),
            normalizedUpdatesDir.resolve("bin").resolve("natives")
        );
        ...
        if (nativesDir != null) {
            ...
            command.add("-Dorg.lwjgl.librarypath=" + nativesDir.toString());
        }
        ...
        if ("net.minecraft.client.Minecraft".equals(mainClass)) {
            // Compatibility fallback for very old client layouts.
            ...
            command.add("1.6.4");
            command.add("--gameDir");
            command.add(clientDir.toString());
        ...
        LogHelper.debug("Offline commandline: %s", command);
    ...
    private static Path resolveClientDir(Path updatesDir, String profileName) throws IOException {
        ...
        throw new IOException("Local client profile '" + profileName + "' was not found under " + updatesDir
            + ". Offline mode can only start files that are already installed.");
    }
    ...
    private static List<Path> collectClassPath(Path updatesDir, Path clientDir) throws IOException {
        ...
        if (Files.isDirectory(libraries) && !libraries.startsWith(clientDir)) {
            scanJars(libraries, entries);
        }
        Path lib = updatesDir.resolve("lib");
        if (Files.isDirectory(lib) && !lib.startsWith(clientDir)) {
            scanJars(lib, entries);
        }
        ...
        Collections.sort(result, new Comparator<Path>() {
            ...
            public int compare(Path left, Path right) {
                int leftPriority = classPathPriority(left);
                int rightPriority = classPathPriority(right);
    ...
    private static void addDirectoryIfPresent(Set<Path> entries, Path directory) {
        if (Files.isDirectory(directory)) {
            entries.add(directory.toAbsolutePath().normalize());
        }
    }
    ...
    private static void scanJars(Path root, final Set<Path> entries) throws IOException {
        Files.walkFileTree(root, new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) {
                String name = file.getFileName().toString().toLowerCase(Locale.ROOT);
                if (name.endsWith(".jar") || name.endsWith(".zip")) {
                    entries.add(file.toAbsolutePath().normalize());
                }
                return FileVisitResult.CONTINUE;
            }
        });
    ...
    private static int classPathPriority(Path path) {
        ...
        if (name.contains("launchwrapper")) {
            return 0;
        }
    ...
    private static boolean containsClass(List<Path> classPath, String className) {
        for (Path entry : classPath) {
            if (Files.isDirectory(entry)) {
                if (Files.isRegularFile(entry.resolve(Paths.get(className)))) {
                    return true;
                }
                continue;
            }
            ...
            try (JarFile jar = new JarFile(entry.toFile())) {
                ...
                // Non-JAR ZIPs are harmless and simply skipped.
            }
        }
        return false;
    }
    ...
    private static Path resolveJavaBinary() throws IOException {
        ...
        if (!Files.isRegularFile(javaBin)) {
            throw new IOException("Java binary was not found: " + javaBin);
        }
