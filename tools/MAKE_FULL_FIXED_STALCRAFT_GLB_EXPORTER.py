...
# -*- coding: utf-8 -*-
"""
ОДНОКЛИЧНЫЙ ГЕНЕРАТОР ПОЛНОГО ИСПРАВЛЕННОГО ФАЙЛА
STALCRAFT GLB Armor Exporter 0.4.3

1. Положи этот файл рядом с blender_stalcraft_glb_export.py.
2. Запусти его обычным Python 3 или через Blender > Scripting > Run Script.
3. Рядом появится ПОЛНЫЙ исправленный файл:
   blender_stalcraft_glb_export_0.4.3_FIXED.py

Исходник не перезаписывается; создаётся резервная копия.
"""
...
def find_source():
    ...
    raise FixError(
        "Не найден %s. Положи этот генератор рядом с исходным exporter-файлом "
        "или перетащи исходный .py на генератор." % SOURCE_NAME
    )
...
def resolve_runtime_config_path(raw_path):
    """
    Accept the Minecraft instance root, its config directory, or the exact
    stalcraftglb.cfg file. This prevents config/config/stalcraftglb.cfg.
    """
...
def verify_written_config(filepath, settings, expected_index):
    """Do not report success unless name, ID and armorCount were really saved."""
...
'''
    ...
    new_runtime = '''        runtime_cfg = resolve_runtime_config_path(props.minecraft_root)
        runtime_index = upsert_config(runtime_cfg, settings)
        verify_written_config(runtime_cfg, settings, runtime_index)
    ...
    old_save = '''    temp_path = filepath + ".tmp"
    with open(temp_path, "w", encoding="utf-8", newline="\\n") as handle:
        handle.write(text.rstrip() + "\\n")
    os.replace(temp_path, filepath)
    return target_index'''
    new_save = '''    if os.path.isfile(filepath):
        try:
...
def main():
    ...
    print()
    print("ГОТОВО — ПОЛНЫЙ ИСПРАВЛЕННЫЙ ФАЙЛ СОЗДАН")
    print("Исходник:   " + source_path)
    print("Резервная:  " + backup_path)
    print("Исправлен:  " + output_path)
    print()
    print("В Blender установи именно:")
    print("  " + OUTPUT_NAME)
    print()
    print("Minecraft / config Path принимает:")
    print("  ...\\classic1.6.4")
